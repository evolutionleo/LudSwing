function handlePacket(pack) {
	...
	switch(cmd) {
		...
		case "player pos":
			...
			if (player.last_updated <= data.last_updated) {
				player.remote = true
				player.last_updated = data.last_updated
				player.x = data.x
				player.y = data.y
				player.spd.x = data.spd.x
				player.spd.y = data.spd.y
				player.alarm[0] = player.max_ping_interval
			}
		...
		case "player controls":
			...
			//if (player.last_updated <= data.last_updated) {
			//with(player) {
			player.kright = data.kright
			player.kleft = data.kleft
			player.kdown = data.kdown
			player.kup = data.kup
				
			player.kjump = data.kjump
			player.kjump_hold = data.kjump_hold
			//}
			//}
